...
const FridgePage = () => {
  ...
  const handleOpenEdit = (item: any) => {
    setEditingItem(item); setForm({ ...item }); setIsModalOpen(true);
  };

  const selectPredefined = (item: any) => {
    const today = new Date();
    const expiry = new Date(today.setDate(today.getDate() + (item.defaultExpiryDays || item.expiry || 7)));
    setForm({ ...form, name: item.name, category: item.category, unit: item.defaultUnit || item.unit || '개', expiryDate: expiry.toISOString().split('T')[0] });
  };
  ...
  const handleSave = () => {
    ...
    if (editingItem) updateIngredient(editingItem.id, { ...form, image: icon });
    else addIngredient({ ...form, id: Date.now().toString(), image: icon, storage: predefinedInfo?.defaultStorage || 'FRIDGE' } as any);
    ...
    <div className="min-h-screen bg-[#f8f9fa] px-5 pt-6 pb-24">
      ...
      <div className="bg-white p-2 rounded-xl border border-gray-100 mb-4 sticky top-[70px] z-10 shadow-sm">
         <div className="relative mb-2">
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="내 냉장고 검색" className="w-full bg-gray-50 border-none rounded-lg py-2 pl-9 pr-4 text-sm focus:ring-1 focus:ring-[#FF6B6B]" />
         ...
         <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
            ...
            {CATEGORIES.map(cat => <button key={cat.id} onClick={() => setFilterCat(cat.id)} className={`shrink-0 px-3 py-1 rounded-md text-xs font-bold ${filterCat === cat.id ? 'bg-[#FF6B6B] text-white' : 'bg-gray-100 text-gray-500'}`}>{cat.label}</button>)}
         </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {filteredItems.map(item => {
          ...
          return (
            <div key={item.id} onClick={() => handleOpenEdit(item)} className="bg-white p-3 rounded-xl border border-gray-100 flex flex-col items-center justify-between cursor-pointer active:scale-[0.98] transition-transform text-center relative h-32">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-5 animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-2xl p-6 animate-slide-up h-[85vh] flex flex-col">
             ...
             <div className="flex-1 overflow-y-auto pr-1">
                {!editingItem && (
                  <div className="bg-orange-50 p-4 rounded-2xl border border-orange-100 mb-6">
                    ...
                    <input value={modalSearch} onChange={e => setModalSearch(e.target.value)} placeholder="재료명 검색 (예: 계란, ㅇㅇ)" className="w-full border p-3 rounded-xl bg-white text-sm mb-3 focus:border-[#FF6B6B] outline-none shadow-sm" />
                    ...
                    <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                      {filteredPredefined.map(p => (
                        <button key={p.name} onClick={() => selectPredefined(p)} className={`flex flex-col items-center justify-center border rounded-xl p-2 transition-all ${form.name === p.name ? 'bg-white border-[#FF6B6B] ring-2 ring-[#FF6B6B] ring-opacity-50' : 'bg-white border-orange-100 hover:border-orange-300'}`}>
                ...
                <div className="space-y-5">
                  {(form.name || editingItem) && <div className="text-center pb-2 border-b border-gray-100"><span className="text-2xl font-black text-gray-800">{form.name}</span><span className="text-xs text-gray-400 ml-2 block mt-1">{CATEGORY_LABELS[form.category]}</span></div>}
             ...
             <button onClick={handleSave} className="w-full bg-[#FF6B6B] text-white py-4 rounded-2xl font-bold mt-4 shadow-lg shadow-orange-200 hover:bg-[#FF5252] transition-colors shrink-0 text-lg">{editingItem ? '수정 완료' : '냉장고에 넣기'}</button>
          </div>
        </div>
      )}
    </div>
